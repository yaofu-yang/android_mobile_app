...
import edu.neu.mad_sea.yaofuyang.ReadRecipesListActivity

class ReadRecipesListActivity : AppCompatActivity(), LoaderManager.LoaderCallbacks<Cursor> {
    // Note: The first column in all tables must be _id, or load/reset cursor methods will not work.
    private lateinit var adapter: SimpleCursorAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_read_recipes_list)
        val recipesList = findViewById<ListView>(R.id.recipes_abridged_listView)
        ...
        recipesList.onItemClickListener = OnItemClickListener { parent, view, position, id ->
            ...
            val intent = Intent(this@ReadRecipesListActivity, ReadRecipeActivity::class.java)
            intent.putExtra("recipe_id", recipeId.text.toString())
            intent.putExtra("recipe_name", recipeName.text.toString())
            this@ReadRecipesListActivity.startActivity(intent)
        }
    ...
    override fun onCreateLoader(id: Int, args: Bundle?): Loader<Cursor> {
        return CursorLoader(
            this,
            RecipeListContentProvider.CONTENT_URI, FROM, null, null,
            null
        )
    }
